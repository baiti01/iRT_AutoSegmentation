...
class UnetSkipConnectionBlock(nn.Module):
    def __init__(self,
        ...
        downconv_no_norm = conv_layer_no_norm(input_channels, inner_channels, kernel_size=3, stride=2, padding=1,
                                              bias=True)
        ...
        if outermost:
            upconv = nn.Sequential(*[nn.Upsample(scale_factor=2.0, mode=upsampling_mode, align_corners=True),
                                     conv_layer_no_norm(inner_channels * 2, outer_channels, kernel_size=3, stride=1,
                                                        padding=1,
                                                        bias=True)])

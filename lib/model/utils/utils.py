...
def get_scheduler(optimizer, option):
    ...
    if option.lr_policy == 'linear':
        ...
        scheduler = lr_scheduler.CosineAnnealingLR(optimizer, T_max=option.niter, eta_min=0,
                                                   last_epoch=option.last_iteration)
    elif option.lr_policy == 'poly':
        def poly_lr(iteration):
            return (1 - iteration / option.niter_decay) ** option.poly_exponent

        scheduler = lr_scheduler.LambdaLR(optimizer, lr_lambda=poly_lr, last_epoch=option.last_iteration)
    else:
